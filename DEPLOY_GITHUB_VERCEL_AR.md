...
## الخطوة 2 — رقِّ قاعدة البيانات القديمة مرة واحدة

من Supabase افتح **SQL Editor**، ثم افتح الملف:

```text
RUN_ONCE_SAFE_DATABASE_UPGRADE.sql
```

انسخ محتواه بالكامل وشغّله مرة واحدة.

الملف لا يحذف بيانات الطلاب أو الجلسات. قبل التعديل، ينشئ نسخًا داخل schema خاصة اسمها:

```text
edubia_backup
```

ويضع بها نسخة من الجداول القديمة، ثم يضيف دعم الحسابات المتعددة والحضور.
...
4. نزّل تقرير Excel شهريًا.
5. افتح رابط `/follow-up` مباشرة ثم اعمل Refresh للتأكد من عدم ظهور 404.
